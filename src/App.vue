...
<template>
  <div class="min-h-screen flex flex-col bg-bimbo-50">
    <header class="fixed top-0 left-0 w-full bg-gradient-to-r from-bimbo-400 via-bimbo-200 to-bimbo-400 shadow-md z-10 border-b border-bimbo-200">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
          ...
          <!-- Sélecteur de langue à droite avec <img> SVG -->
          <div class="flex items-center space-x-2 ml-4">
            <button @click="locale = 'en'" :class="[locale === 'en' ? 'ring-2 ring-bimbo-400' : '', 'rounded-full p-1 transition']" title="English">
              <img src="./assets/flag-gb.svg" alt="GB" class="w-7 h-5 rounded shadow-sm" />
            </button>
            <button @click="locale = 'fr'" :class="[locale === 'fr' ? 'ring-2 ring-bimbo-400' : '', 'rounded-full p-1 transition']" title="Français">
              <img src="./assets/flag-fr.svg" alt="FR" class="w-7 h-5 rounded shadow-sm" />
            </button>
          </div>
        </div>
      </div>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 mt-24 flex flex-col justify-center items-center">
      <PlaylistManager v-if="currentView === 'player'" />
      <Editor v-else />
    </main>
  </div>
</template>
